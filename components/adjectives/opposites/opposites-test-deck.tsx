...
import type { Opposite } from "@/lib/db/opposites";
...
export function OppositesTestDeck({
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const correct = normalize(input) === normalize(current.opposite);
    setResults((r) => [...r, { pair: current, userAnswer: input.trim(), correct }]);
    setAnswered(true);
  };

  const restart = () => {
    setOrder(shuffle(pairs));
    setIndex(0);
    setInput("");
    setAnswered(false);
    setResults([]);
        ...
        <div className="relative z-10 flex w-full max-w-lg flex-col items-center gap-6 text-center">
          ...
          {wrongResults.length === 0 ? (
            <p className="text-muted-foreground">
              Perfect score — every opposite correct!
            </p>
          ) : (
            <div className="w-full overflow-x-auto rounded-2xl border">
              <table className="w-full min-w-[28rem] text-left text-sm">
                ...
                <tbody>
                  {wrongResults.map((r) => (
                    <tr key={r.pair.id} className="border-t">
                      <td className="px-4 py-2">{r.pair.word}</td>
          ...
          <AnimatePresence>
            {answered && (
              ...
              >
                ...
                {!isCorrect && (
                  <span>
                    The right answer is <strong>{current.opposite}</strong>.
                  </span>
                )}
              </motion.div>
            )}
