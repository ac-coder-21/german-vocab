...
function buildQuestions(verbs: Verb[]): Question[] {
  const questions: Question[] = [];
  for (const verb of verbs) {
    for (const { key, label } of PRONOUNS) {
      const form = verb[key];
      if (form) questions.push({ verb, pronounLabel: label, correctForm: form });
    }
  }
...
export function TestConjugationDeck({
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const correct = normalize(input) === normalize(current.correctForm);
    setResults((r) => [...r, { question: current, userAnswer: input.trim(), correct }]);
    setAnswered(true);
  };

  const restart = () => {
    setOrder(buildQuestions(verbs));
    setIndex(0);
    setInput("");
    setAnswered(false);
    setResults([]);
      ...
      <div className="relative flex flex-1 flex-col items-center justify-center gap-4 overflow-hidden px-6 text-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-4">
          ...
          <div className="flex gap-3">
            <Button
              render={<Link href="/categories/verbs/manage" />}
              nativeButton={false}
              variant="outline"
            >
              Manage verbs
        ...
        <div className="relative z-10 flex w-full max-w-lg flex-col items-center gap-6 text-center">
          ...
          {wrongResults.length === 0 ? (
            <p className="text-muted-foreground">
              Perfect score — every conjugation correct!
            </p>
          ) : (
  ...
  return (
    <div className="relative flex flex-1 flex-col items-center justify-center gap-8 overflow-hidden px-6 py-16">
      ...
      <div className="relative z-10 flex w-full max-w-sm flex-col items-center gap-6">
        ...
        <motion.div
          key={`${current.verb.id}-${current.pronounLabel}-${index}`}
        ...
        >
          <span className="text-sm font-medium text-muted-foreground">
            Conjugate for &quot;{current.pronounLabel}&quot;
          </span>
          <p className="text-3xl font-bold tracking-tight text-balance">
            {current.verb.german}
          </p>
          <p className="text-sm text-muted-foreground">{current.verb.english}</p>
          ...
          <form onSubmit={handleSubmit} className="mt-2 flex w-full flex-col gap-3">
            <input
              ...
              readOnly={answered}
              placeholder={`${current.pronounLabel} …`}
          ...
          <AnimatePresence>
            {answered && (
              ...
              >
                ...
                {!isCorrect && (
                  <span>
                    The right answer is <strong>{current.correctForm}</strong>.
                  </span>
                )}
              </motion.div>
            )}
