...
export function LearnConjugationDeck({
  verbs,
  setNumber,
}: {
  verbs: Verb[];
  ...
  const hasConjugation = current && current.ich !== null;
          ...
          <AnimatePresence mode="popLayout" custom={direction} initial={false}>
            {!finished ? (
              <motion.div
                ...
                className="absolute inset-0 flex cursor-pointer touch-pan-y select-none flex-col items-center gap-4 rounded-3xl border bg-card/80 p-8 text-center shadow-lg backdrop-blur active:cursor-grabbing"
              >
                ...
                {revealed ? (
                  hasConjugation ? (
                      ...
                      {PRONOUNS.map(({ key, label }) => (
                        ...
                        >
                          ...
                          <span className="flex items-center gap-1 font-semibold">
                            ...
                            <button
                              type="button"
                              aria-label={`Hear ${label} form`}
                              onClick={(e) => {
                                e.stopPropagation();
                                if (current[key]) speakGerman(current[key]!);
                              }}
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      No conjugation added for this verb yet.
                    </p>
                  )
                ) : (
                  <span className="mt-auto text-xs text-muted-foreground">
                    Tap to reveal conjugation · swipe for next
                  </span>
                )}
              </motion.div>
            ) : (
