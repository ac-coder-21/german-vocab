...
export function TestGenderDeck({
  nouns,
  setNumbers,
}: {
  nouns: Noun[];
  ...
  const [order, setOrder] = useState(() => shuffle(nouns));
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const correct = normalize(input) === normalize(current.artikel);
    setResults((r) => [...r, { noun: current, userAnswer: input.trim(), correct }]);
    setAnswered(true);
  };

  const restart = () => {
    setOrder(shuffle(nouns));
    setIndex(0);
    setInput("");
    setAnswered(false);
    setResults([]);
        ...
        <div className="relative z-10 flex w-full max-w-lg flex-col items-center gap-6 text-center">
          ...
          {wrongResults.length === 0 ? (
            <p className="text-muted-foreground">
              Perfect score — every gender correct!
            </p>
          ) : (
            <div className="w-full overflow-x-auto rounded-2xl border">
              <table className="w-full min-w-[28rem] text-left text-sm">
                ...
                <tbody>
                  {wrongResults.map((r) => (
                    <tr key={r.noun.id} className="border-t">
                      <td className="px-4 py-2">{r.noun.german}</td>
          ...
          <div className="mt-2 flex gap-3">
            ...
            <Button render={<Link href="/categories/nouns" />} nativeButton={false}>
              <ArrowLeft className="size-4" />
              Back to nouns
            </Button>
          </div>
          ...
          <AnimatePresence>
            {answered && (
              ...
              >
                ...
                {!isCorrect && (
                  <span>
                    The right answer is <strong>{current.artikel}</strong>.
                  </span>
                )}
              </motion.div>
            )}
