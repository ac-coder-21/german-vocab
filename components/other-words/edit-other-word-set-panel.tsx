...
import {
  addWordToSetAction,
  deleteOtherWordAction,
  deleteOtherWordSetAction,
  updateOtherWordAction,
  type ActionState,
} from "@/app/categories/other-words/manage/actions";
...
export function EditOtherWordSetPanel({
  sets,
  selectedSet,
  words,
}: {
...
function WordRow({ word }: { word: OtherWord }) {
  const updateWithId = updateOtherWordAction.bind(null, word.id);
  const deleteWithId = deleteOtherWordAction.bind(null, word.id);
  ...
  return (
    ...
    >
      <input name="german" defaultValue={word.german} className={fieldClass} />
      <input name="english" defaultValue={word.english} className={fieldClass} />
...
function DeleteSetButton({ setNumber }: { setNumber: number }) {
  const boundAction = deleteOtherWordSetAction.bind(null, setNumber);
